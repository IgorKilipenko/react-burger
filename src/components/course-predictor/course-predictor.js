...
const getRatio = (current, previous = 0) =>
  (((current - previous) / (current < previous ? previous : current)) * 100).toFixed(2)
...
export default function CoursePredictor() {
  ...
  const currentElement = currentCourse && (
    <output className={coursePredictorStyles.currentElement}>{currentCourse} ₽</output>
  )
  ...
  return (
    <section className={coursePredictorStyles.card}>
      ...
      <p className={coursePredictorStyles.caption}>За 1 $</p>
